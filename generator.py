...
class Booster:
    def __init__(self) -> None:
        ...
        self.config         = json.load(open('./input/config.json', 'r'))
        self.proxies        = open('./input/proxies.txt', 'r').read().splitlines()
        if not len(self.proxies):
            print(f"\n{Fore.RED}[{Fore.RESET}{Fore.MAGENTA}!{Fore.RESET}{Fore.RED}]{Fore.RESET} No proxy found in proxy folder.\n{Fore.RED}[{Fore.RESET}{Fore.MAGENTA}!{Fore.RESET}{Fore.RED}]{Fore.RESET} Please edit the config file in the input folder.") # err if no proxy in input/proxies.txt
            time.sleep(10)
            os._exit(0)
    ...
    def __gen__(self, captcha_token: str, proxy: str) -> None:
        try:
            ...
            token = r.json()['token']
            self.captcha_tokens.append(captcha_token)
            with open("./input/tokens.txt", "a") as x: x.write(f'{token}\n')
            with open("./input/accounts.txt", "a") as x: x.write(f'{email}:{self.config["password"]}:{token}\n')
